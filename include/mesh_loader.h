...
void readPlyFile(const std::string &mesh_path, std::vector<float3> &vverts, std::vector<uint3> &vfaces,
                std::vector<float3> &vnormals, std::vector<float6> &vtexuv)
{
    std::cout << "........................................................................\n";
    std::cout << "Reading PLY file: " << mesh_path << std::endl;
    ...
    try
    {
        ...
        std::shared_ptr<tinyply::PlyData> pvertices, pfaces, pnormals, ptexcoords;
        ...
        if (pnormals) std::cout << "\tRead " << pnormals->count  << " total normals "<< std::endl;
        if (pfaces)   std::cout << "\tRead " << pfaces->count  << " total faces "<< std::endl;
        if (ptexcoords)   std::cout << "\tRead " << ptexcoords->count  << " total tex coords "<< std::endl;

        if (pfaces->count != ptexcoords->count)
        {
            std::cerr << "\tSHOULD HAVE AS MANY TEXTURE COORDS AS FACES!!!\n";
            std::cerr << "\tCHECK YOUR PLY FILE...\n";
            exit(-1);
        }
        ...
        const size_t numTexBytes = ptexcoords->buffer.size_bytes();
        vtexuv.resize(ptexcoords->count);
        std::memcpy(vtexuv.data(), ptexcoords->buffer.get(), numTexBytes);
        ...
        std::cout << "\n > Number of vertices : " << vverts.size() << std::endl;
        std::cout << "> Number of faces : " << vfaces.size() << std::endl;
        std::cout << "> Number of tex coords : " << vtexuv.size() << std::endl;
